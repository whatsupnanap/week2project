...
//Rare exceptions to indicate a bug in the _wk2_weather code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_wk2_weather = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_wk2_weather]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.wk2_weather.module"
